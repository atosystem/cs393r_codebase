...
class Pose2Df {
  ...
  public:
    ...
    Pose2Df(const float _angle, const Eigen::Vector2f& _translation) : angle(_angle), translation(_translation) {}

    bool operator==(const Pose2Df& other) const {
      return translation == other.translation && angle == other.angle;
    }

    Pose2Df operator - (Pose2Df p2)
    {
        Pose2Df temp;
        temp.translation = translation - p2.translation;
        temp.angle = math_util::AngleDiff(angle,p2.angle);
    ...
    Pose2Df operator + (Pose2Df p2)
    {
        Pose2Df temp;
        temp.translation = translation + p2.translation;
        temp.angle = math_util::AngleMod(angle + p2.angle);

        return temp;
    }

    void Set(const float new_angle, const Eigen::Vector2f& new_translation) {
      angle = new_angle;
