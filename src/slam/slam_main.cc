...
DEFINE_string(rosbag_fp, "none", "Filename of the ROS bag");
DEFINE_double(rosbag_start, -1, "start sec for ROS bag, -1 for none");
DEFINE_double(rosbag_end, -1, "end sec for ROS bag, -1 for none");
...
void playBag() {
    ...
    RateLoop loop(50.0);
    bool clip_start = (FLAGS_rosbag_start >=0);
    bool clip_end = (FLAGS_rosbag_end >=0);
    ...
    { 
        ...
        if (clip_start && message.getTime() <= ros::Time(FLAGS_rosbag_start)) {
          continue;
        }
        if (clip_end && message.getTime() >= ros::Time(FLAGS_rosbag_end)) {
          break;
        }
        ros::spinOnce();
        ROS_INFO_STREAM("Play bag @" << message.getTime());
